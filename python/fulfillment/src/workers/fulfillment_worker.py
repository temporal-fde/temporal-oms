...
async def build_fulfillment_worker() -> Worker:
    ...
    _log.info("[%s] connected — workflows: ShippingAgent | activities: build_system_prompt, call_llm, get_location_events | nexus: ShippingAgent", _TASK_QUEUE)
    llm_activities = LlmActivities()
    location_activities = LocationEventsActivities()
    return Worker(
        client,
