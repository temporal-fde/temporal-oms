...
- [ ] `ShippingAgentImpl.recommend_shipping_option` receives a properly decoded `RecommendShippingOptionRequest` (Pydantic) when called from Java.
...
Java fulfillment.Order workflow
  │  encodes RecommendShippingOptionRequest as json/protobuf  ← or binary/protobuf
  │
  ▼ Nexus call (cross-namespace)
Python fulfillment worker
  │  custom converter (json OR binary slot) → RecommendShippingOptionRequest (_p2p)
  │
  ▼ ShippingAgentImpl.recommend_shipping_option(ctx, input: RecommendShippingOptionRequest)
  │  (pure Pydantic — no pb2 visible here)
  │
  ▼ returns RecommendShippingOptionResponse (_p2p)
  │  custom converter encodes _p2p → json/protobuf  (Python always emits json)
  │
  ▼ Java decodes json/protobuf → RecommendShippingOptionResponse proto
```
...
from acme.fulfillment.domain.v1.shipping_agent_p2p import RecommendShippingOptionRequest
from acme.fulfillment.domain.v1.shipping_agent_pb2 import RecommendShippingOptionRequest as _pb2_RecommendShippingOptionRequest
# ... one pair of imports per message type across all generated packages

REGISTRY: dict[type[BaseModel], type[Message]] = {
    RecommendShippingOptionRequest: _pb2_RecommendShippingOptionRequest,
    # ...
}
```
...
- **JSON round-trip:** Construct a `RecommendShippingOptionRequest`, call `PydanticJsonProtoPayloadConverter().to_payload`, assert `encoding == b"json/protobuf"` and `messageType` is set, then call `from_payload` with the Pydantic type as `type_hint`, assert the result equals the original.
...
- **Nexus boundary smoke test:** In the existing `test_shipping_agent.py`, verify that `ShippingAgentImpl.recommend_shipping_option` can be called with a payload encoded by the converter and returns a correctly decoded `RecommendShippingOptionResponse`. (Uses `temporalio.testing.WorkflowEnvironment` — no real Java process required; encode the input manually with `proto_pydantic_data_converter` before passing to the Nexus test helper.)
...
- [ ] Payload inspection confirms `messageType` header matches the fully-qualified proto name (e.g. `acme.fulfillment.domain.fulfillment.v1.RecommendShippingOptionRequest`)
